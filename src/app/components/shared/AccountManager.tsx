...
import CabinetSection from '../layout/CabinetSection';
import { 
  User, 
  LogOut, 
  Settings, 
...
export default function AccountManager() {
  ...
  const sections = [
    ...
    { id: 'cabinets' as AccountSection, label: 'Кабинеты', icon: Users },
    { id: 'security' as AccountSection, label: 'Безопасность', icon: Shield },
  ];

  return (
    <div className="space-y-6">
      {/* Заголовок с кнопкой выхода - адаптация для мобильных */}
      <div className="liquid-glass rounded-2xl md:rounded-3xl border-2 border-gray-300 p-4 md:p-8 shadow-xl">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3 md:gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-lg md:text-2xl font-bold text-gray-900 truncate">{user.name || 'Пользователь'}</h3>
              <p className="text-sm md:text-base text-gray-700 truncate">{user.email}</p>
              <div className="flex items-center gap-2 mt-1 md:mt-2 flex-wrap">
                <span className="text-xs md:text-sm text-green-700 font-semibold flex items-center gap-1">
          ...
          <button
            ...
            className="w-full md:w-auto px-4 py-2 md:px-6 md:py-3 bg-red-100 hover:bg-red-200 text-red-700 font-semibold rounded-lg transition-all flex items-center justify-center gap-2 border-2 border-red-300 hover:border-red-400 shadow-lg text-sm md:text-base"
            disabled={loading}
          >
      ...
      {/* Навигация по разделам - адаптация для мобильных */}
      <div className="liquid-glass rounded-2xl md:rounded-3xl border-2 border-gray-300 p-2 md:p-3 shadow-xl">
          ...
          {sections.map((section) => {
            ...
            return (
              <button
                ...
                className={`p-2 md:p-3 rounded-lg transition-all duration-300 flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2 font-semibold text-xs md:text-base ${
      ...
      {/* Контент разделов - адаптация для мобильных */}
      <div className="liquid-glass rounded-2xl md:rounded-3xl border-2 border-gray-300 p-4 md:p-8 shadow-xl">
