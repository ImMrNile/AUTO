...
export default function InfographicCreator({ 
  ...
  const checkExistingInfographics = async () => {
    try {
      ...
      clientLogger.error('Ошибка загрузки существующих инфографик:', error);
    }
  };
...
const startInfographicCreation = async () => {
  clientLogger.log('🚀 Начинаем создание инфографики...');
  ...
  try {
    clientLogger.log('📋 Подготавливаем FormData...');
    ...
    additionalImages.forEach((file, index) => {
      clientLogger.log(`📸 Добавляем файл ${index}: ${file.name} (${file.size} bytes, type: ${file.type})`);
    ...
    clientLogger.log('📊 FormData содержит:');
    for (const [key, value] of formData.entries()) {
      if (value instanceof File) {
        clientLogger.log(`- ${key}: File(${value.name}, ${value.size} bytes, ${value.type})`);
      } else {
        clientLogger.log(`- ${key}: "${value}"`);
      }
    }
    ...
    clientLogger.log('🌐 Отправляем запрос на сервер...');
    clientLogger.log(`📍 URL: /api/products/${productId}/infographic`);
    ...
    clientLogger.log('🔧 Параметры запроса:', {
    ...
    clientLogger.log('📨 Ответ сервера:', {
    ...
    clientLogger.log('📄 Тело ответа (первые 500 символов):', responseText.substring(0, 500));

    if (!response.ok) {
      clientLogger.error('❌ HTTP ошибка:', response.status, response.statusText);
      clientLogger.error('❌ Полный ответ:', responseText);
    ...
    try {
      ...
      clientLogger.log('✅ JSON успешно распарсен:', {
      ...
      clientLogger.error('❌ Проблемный текст:', responseText);
    ...
    if (data.success) {
      ...
      clientLogger.log('🎉 Инфографика создана успешно!');
      clientLogger.log(`📊 Создано: ${infographicsCount} изображений`);
      clientLogger.log(`🎯 Качество: ${Math.round(qualityScore * 100)}%`);
    } else {
      clientLogger.error('❌ Сервер вернул ошибку:', data);
      throw new Error(data.error || 'Неизвестная ошибка сервера');
    }

  } catch (error) {
    clientLogger.error('❌ Критическая ошибка создания инфографики:', error);
    setCreationProgress(0);
    ...
    if (error instanceof Error) {
      ...
      clientLogger.error('❌ Stack trace:', error.stack);
    }
