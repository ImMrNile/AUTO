...
export default function ProductForm({ onSuccess }: ProductFormProps): JSX.Element {
  ...
  const [categorySizeInfo, setCategorySizeInfo] = useState({
    hasSizeCharacteristics: false,
    isChecking: false,
    sizeCharacteristics: []
  });
  ...
  const checkCategorySizeSupport = useCallback(async (category: WBSubcategory | null) => {
    if (!category) {
      setCategorySizeInfo({
        hasSizeCharacteristics: false,
        isChecking: false,
    ...
    setCategorySizeInfo(prev => ({ ...prev, isChecking: true }));
    
    try {
      const response = await fetch(`/api/categories/${category.id}/characteristics/size-check`);
      const data = await response.json();
      
      setCategorySizeInfo({
        hasSizeCharacteristics: data.hasSizeCharacteristics || false,
        isChecking: false,
        sizeCharacteristics: data.sizeCharacteristics || []
      });
      
      // Сброс размеров если категория их не поддерживает
      if (!data.hasSizeCharacteristics && formData.hasVariantSizes) {
        setFormData(prev => ({
          ...prev,
          hasVariantSizes: false,
          variantSizes: []
...
const handlePublishToWildberries = async () => {
  ...
  const handleCategorySelect = (category: WBSubcategory | null) => {
    ...
    checkCategorySizeSupport(category); // Добавить эту строку
          ...
          <div className="space-y-4">
            {currentStep === 1 && (
              <Step1BasicInfo
                ...
                cabinets={cabinets}
                categorySizeInfo={categorySizeInfo}
