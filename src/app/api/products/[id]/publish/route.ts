  ...
  try {
    ...
    if (finalStatus === 'PUBLISHED') {
      ...
      try {
        ...
        // Реальный запрос к Wildberries API
        const wbApiResponse = await fetch('https://suppliers-api.wildberries.ru/content/v2/cards', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': cabinet.apiToken // Используем токен из кабинета
          },
        ...
        return NextResponse.json({ 
          ...
          message: 'Данные товара сохранены, но публикация на WB не удалась. Можете повторить позже.'
        }, { status: 500 });
      }
    } else {
