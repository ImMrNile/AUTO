...
ВАЖНО: Без воды, только суть.`.trim();
  ...
  private filterCharacteristicsForAI(characteristics: CategoryCharacteristic[]): CategoryCharacteristic[] {
    const EXCLUDED_WB_IDS = new Set([
      ...
      // ИКПУ и коды
      15001650, 15001706,
      // 🔥 НОВОЕ: Критически важные характеристики (только ручной ввод)
      9623,    // Гарантийный срок - юридически важно
      167966,  // Срок эксплуатации - юридически важно  
      14177451, // Страна производства - важно для таможни
      72443,   // Хрупкость - важно для логистики
      14177453 // SKU - системное поле
    ]);
